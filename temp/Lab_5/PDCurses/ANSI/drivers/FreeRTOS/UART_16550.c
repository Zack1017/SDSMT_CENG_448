...
void UART_16550_configure(int UART,int baud,int parity,int bits,int stop_bits)
{
  ...
  LCR_t lcr = {0};   // IMPORTANT: initialize all bitfields to zero

  lcr.DLAB = 1;
  uart[UART].dev->LCR = lcr;

  uart[UART].dev->DLL = divisor & 0xFF;
  uart[UART].dev->DLH = divisor >> 8;
  ...
  {
    lcr.PEN = 1;
    lcr.EPS = (parity == UART_PARITY_EVEN) ? 1 : 0;
  }
  ...
  lcr.BREAK = 0;   // make sure TX is not held in break
  uart[UART].dev->LCR = lcr;
